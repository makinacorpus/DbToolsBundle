...
**Almost every option can be configured throught environment variables.**
...
[`restore_binaries`](#restorer-binaries) |
[`restore_options`](#restorer-options) |
...
```ini [Environment]
DBTOOLS_ANONYMIZER_PATH="./vendor/makinacorpus/db-tools-bundle/src/Anonymizer,./src/Anonymization/Anonymizer"
```
:::
...
```ini [Environment]
DBTOOLS_BACKUP_EXCLUDED_TABLES="table1,table2"
```
:::

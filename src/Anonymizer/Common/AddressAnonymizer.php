 ...
 */
class AddressAnonymizer extends AbstractAnonymizer
{
    /**
     * @var Array<string[]> ['street_address', 'secondary_address', 'postal_code', 'locality', 'region', 'country']
     */
    ...
    {
        $stringType = Type::getType('string');

        $this->connection
            ->createSchemaManager()
            ->createTable(new Table(
                $this->getSampleTableName(),
                \array_map(fn($column) => new Column($column, $stringType), $this->columns)
            ))
        ;

        $this->connection->beginTransaction();
        try{
            foreach($this->sample as $values) {
                $this->connection
                    ->createQueryBuilder()
                    ->insert($this->getSampleTableName())
                    ->values(\array_combine(
                        $this->columns,
                        \array_map(fn($column) => ':'. $column, $this->columns)
                    ))
                    ->setParameters(\array_combine($this->columns, $values))
                    ->executeQuery()
                ;
            }
    ...
    public function anonymize(QueryBuilder $query, Target\Target $target, Options $options): self
    {
        ...
        $columnOptions = \array_filter($this->columns, fn($column) => $options->has($column));

        $random = $this->connection
            ->createQueryBuilder()
            ->select(...\array_map(fn($column) => $this->getSampleTableName() . '.' . $column, $columnOptions))
            ...
            ->where(
                $this->connection->createExpressionBuilder()->notLike(
                    ...
                    $this->getSampleTableName() . '.' . \reset($columnOptions)
                )
            )

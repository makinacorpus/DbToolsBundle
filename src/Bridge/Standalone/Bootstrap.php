...
class Bootstrap
{
    ...
    private static function configGetEnv(array $config): array
    {
        ...
        foreach (['mariadb', 'mysql', 'postgresql', 'sqlite'] as $vendor) {
            $config['backup_binaries'][$vendor] = self::getEnv('DBTOOLS_BACKUP_BINARY_' . \strtoupper($vendor));
            $config['restore_binaries'][$vendor] = self::getEnv('DBTOOLS_RESTORE_BINARY_' . \strtoupper($vendor));
        }
